...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--startup-settle-delay", type=float, default=0.1, help="Seconds to wait before showing the startup dashboard cards")
...
def startup_dashboard_cards(args):
    locale = locale_for_args(args)
    messages, preferred_message_index = build_startup_messages(args, settings="", locale=locale)
    cards = []
...
async def run_live(args) -> None:
    ...
    startup_cards, preferred_card_index = startup_dashboard_cards(args)
    ...
    for idx, card in enumerate(startup_cards):
        update_current_index(Path(args.card_state_file).expanduser(), args.card_state_key, idx)
        await display.show(card, priority="high" if ("当前任务" in card or "Current task" in card) else "normal")
        if idx < len(startup_cards) - 1:
            await asyncio.sleep(args.startup_delay)
    if startup_cards:
        set_cards(Path(args.card_state_file).expanduser(), args.card_state_key, startup_cards, current_index=preferred_card_index)
        await display.show(startup_cards[preferred_card_index], priority="high" if ("当前任务" in startup_cards[preferred_card_index] or "Current task" in startup_cards[preferred_card_index]) else "normal")
