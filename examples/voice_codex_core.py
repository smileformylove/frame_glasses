...
def parse_intent(text: str, wake_word: Optional[str] = None, shortcuts=None) -> BridgeIntent:
    ...
    if any(word in lowered for word in RECENT_TASKS_WORDS):
...
async def execute_intent(args, intent: BridgeIntent) -> Tuple[str, bool]:
    ...
    if intent.action == "task_recent":
        return summarize_recent_tasks(Path(args.task_state_file).expanduser(), locale=locale), False
    if intent.action == "task_previous":
        task = switch_to_previous_task(Path(args.task_state_file).expanduser())
        if not task:
